...
| LLM | `deepseek/deepseek-v4-flash` by default via OpenRouter; optional fallback slugs are explicitly configured |
| Frontend | React 18 + TypeScript + Tailwind CSS + framer-motion; zero-dependency standalone Web Component |
...
- **First time**: a folder URL uses `git clone --depth 1 --filter=blob:none --sparse`; a whole-repository URL uses a shallow blobless clone into `.github_cache/`
...
staging_collection = create_collection(
    name="knowledge-staging-<uuid>",
    metadata={"hnsw:space": "cosine"}
)
# Persist active_index.json only after the staged collection is complete.
```
...
| **Collection** | Versioned staging collections; `active_index.json` points to the active one |
...
- A complete staging collection and BM25 state are built before an atomic active-pointer switch.
- Searches keep a read lease on the prior collection during the switch; a failed rebuild retains the prior index.
- No incremental update — the entire index is still re-embedded each rebuild.
...
3. Filter: keep only vector chunks where cosine distance <= MIN_SIMILARITY (default: 0.5)
4. Result: vector_ranked = [(chunk_id, distance), ...]
```

- `MIN_SIMILARITY` acts as a relevance gate — vector chunks above the threshold are discarded.
...
- RRF avoids an extra LLM call. Evaluate task-specific retrieval quality before deciding whether re-ranking justifies its latency and cost.
...
| Variable | Default | Description |
|----------|---------|-------------|
| `OPENROUTER_MODEL` | `deepseek/deepseek-v4-flash` | Optional comma-separated model list; the default uses DeepSeek V4 Flash only |
...
| `MAX_HISTORY` | `40` messages | Fixed limit: 20 user/assistant turns kept in context |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `CORS_ORIGIN` | *(empty)* | Exact comma-separated allowed origins; empty means same-origin only and `*` is rejected |
| `TRUSTED_PROXY_IPS` | *(empty)* | Direct proxy IP/CIDR list allowed to supply forwarded IP/protocol headers |
| request body | `5 MiB` | Fixed maximum JSON request size for chat and suggest |
...
**Response:** a streamed `text/plain` body. The `X-RAG-Sources` response header carries up to three display-safe retrieved source labels as JSON.
...
7. Plain-text streaming response; frontends render model output as text, not HTML
...
Force-rebuilds the entire knowledge base index. The route is hidden until `REBUILD_SECRET` is set, then requires that value in the `x-rebuild-secret` header.
...
- **Evaluation first**: RRF avoids an extra LLM call. Measure retrieval quality on a task-specific evaluation set before deciding whether re-ranking is worth its added cost and latency.
...
### 12.5 Why Is the Default Cosine-Distance Threshold 0.5?

`MIN_SIMILARITY=0.5` is a starting configuration, not a universal quality guarantee. In Chroma cosine-distance space, lower values are more similar; the implementation excludes vector candidates whose distance is *greater* than this threshold. Tune it against a representative evaluation set for the target knowledge base, and keep zero-score BM25 candidates out of retrieval.
...
- The staged collection uses a fresh `add` operation, so IDs make the build deterministic but do not currently avoid embedding work.
- The implementation swaps complete staged collections rather than deleting the live collection. **Future upgrade path: hash-based change detection to skip re-embedding unchanged documents.**
...
| LLM | 默认通过 OpenRouter 使用 `deepseek/deepseek-v4-flash`；可显式配置其他回退 slug |
| 前端 | React 18 + TypeScript + Tailwind CSS + framer-motion；零依赖独立 Web Component |
...
- **首次**：文件夹 URL 使用 `git clone --depth 1 --filter=blob:none --sparse`；整仓 URL 使用 shallow blobless clone，缓存到 `.github_cache/`
...
# 仅在 staging 集合完整后写入 active_index.json。
```
...
| **索引** | HNSW（分层导航小世界图） |
| **距离度量** | 余弦距离 |
| **集合** | 版本化 staging 集合；`active_index.json` 指向当前活动集合 |
...
### 一致性保证
- 完整的 staging 集合和 BM25 状态构建完成后，才原子切换活动指针。
- 切换期间检索会持有旧集合的读租约；重建失败会保留旧索引。
- 无增量更新——每次重建仍会重新嵌入整个索引。
...
1. 嵌入用户查询 → 1536 维向量
2. ChromaDB.query(query_embeddings=[vector], n_results=CANDIDATE_K)
3. 过滤：仅保留余弦距离 <= MIN_SIMILARITY（默认：0.5）的向量分块
4. 结果：vector_ranked = [(chunk_id, distance), ……]
```

- `MIN_SIMILARITY` 作为相关性门控——距离高于阈值的向量分块被丢弃。
...
- RRF 无需额外 LLM 调用。是否需要重排序，应基于目标任务的检索评测结果权衡其延迟和成本。
...
| 变量 | 默认值 | 描述 |
|------|--------|------|
| `OPENROUTER_MODEL` | `deepseek/deepseek-v4-flash` | 可选的逗号分隔模型列表；默认仅使用 DeepSeek V4 Flash |
...
| `MAX_HISTORY` | `40` 条消息 | 固定限制：上下文中最多保留 20 个用户/助手轮次 |
...
| 变量 | 默认值 | 描述 |
|------|--------|------|
| `CORS_ORIGIN` | *（空）* | 精确的逗号分隔允许 Origin；空表示仅同源，拒绝 `*` |
| `TRUSTED_PROXY_IPS` | *（空）* | 可提供转发 IP/协议头的直接代理 IP/CIDR 列表 |
| 请求体 | `5 MiB` | chat 与 suggest 的固定 JSON 请求体上限 |
...
**响应：** 流式 `text/plain` 正文。`X-RAG-Sources` 响应头以 JSON 形式返回至多三个可展示的检索来源标签。
...
强制重建整个知识库索引。只有设置 `REBUILD_SECRET` 后路由才会暴露，并且必须通过 `x-rebuild-secret` 请求头发送该值。
...
- **先评估再决定**：RRF 避免了一次额外的 LLM 调用。是否值得加入重排序，应根据目标知识库的评测集衡量其额外成本和延迟。
...
### 12.5 为什么默认余弦距离阈值为 0.5？

`MIN_SIMILARITY=0.5` 只是起始配置，不是普适的质量保证。在 Chroma 的余弦距离空间中，数值越小表示越相似；实现会过滤距离**大于**该阈值的向量候选。应基于目标知识库的代表性评测集调参，并确保零分 BM25 候选不进入检索结果。

### 12.6 为什么使用稳定的分块 ID？

分块 ID 是 `(来源, 标题, 内容)` 的 MD5 哈希。这意味着：
- 重新索引未更改的文档会重用稳定 ID。
- staging 集合使用新的 `add` 操作，因此 ID 让构建可复现，但当前不会减少嵌入工作。
- 实现会切换完整 staging 集合，而不是删除活动集合。**未来升级路径：基于哈希的变更检测以跳过未更改文档的重新嵌入。**
